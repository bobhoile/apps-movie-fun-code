...
public class HomeController {
    ...
    private void createAlbums() {
        TransactionStatus txStatus = albumsTransactionManager.getTransaction(null);

        for(Album album : albumFixtures.load()){
            albumsBean.addAlbum(album);
        }
        albumsTransactionManager.commit(txStatus);
    }

    private void createMovies() {
        TransactionStatus txStatus = moviesTransactionManager.getTransaction(null);

        for(Movie movie : movieFixtures.load()){
            moviesBean.addMovie(movie);
        }
        moviesTransactionManager.commit(txStatus);
    }
}
